...
#define PhysicsTools_HepMCCandAlgos_CSA07ProcessIdFWLite_h
  ...
  class csa07ProcessId {
    ...
    public:
      ...
      csa07ProcessId(const fwlite::Event & iEvent, float overallLumi = 1000, std::string csa07EventWeightProducerLabel = "csa07EventWeightProducer") {
    	...
    	procIdH.getByLabel(iEvent, "genEventProcID");
    	int procId = *procIdH;
    	...
    	fwlite::Handle<double> scale;
    	scale.getByLabel(iEvent, "genEventScale");
    	double ptHat = *scale;
    	...
    	if (runOnChowder) {
          ...
          fwlite::Handle<double> filterEffH;
          filterEffH.getByLabel(iEvent, "genEventRunInfo", "FilterEfficiency");
